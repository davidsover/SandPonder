...
try {
	shuffleWorker = new WorkerProxy("Source/Engine/ShuffleWorker.js")
}
catch {
	if (RANDOM == "shuffle") {
...
if (RANDOM === "shuffle") {
	...
	let eventCount = 65536 / 4
	const idRatio = (1 / 2**32) * spaceCount
	const floatEventRatio = spaceCount / eventCount * SPEED_MOD
	const eventRatio = Math.ceil(floatEventRatio)
	const ids = new Uint32Array(eventCount)
	print(floatEventRatio)
	if (floatEventRatio < 1) {
